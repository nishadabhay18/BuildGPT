...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 z-50 flex flex-col h-screen w-72 p-5  dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r brder-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'} `}>
      ...
      {/* <div className={`fixed top-0 left-0 z-50 flex flex-col h-screen w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute ${!isMenuOpen && 'max-md:-translate-x-full'}`}> */}
      ...
      <div onClick={() => { navigate('/community'); setIsMenuOpen(false) }}
        ...
        <div className='flex flex-col text-sm'><p>Community Images</p></div>
      </div>
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-gray-400'>Purchase credits to use builggpt</p>
        </div>
      </div>
      ...
      <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md '>
        ...
        <div className='flex items-center gap-2 text-sm'><p>Dark Mode</p></div>
